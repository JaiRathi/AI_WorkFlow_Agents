...
> On approval, step 0 copies this file to `Hybrid_RAG_QA_Buddy/Plan.md` (final version, as requested).
...
Key fact from recon: `/Users/jaivirsingh/Downloads/Advance_RAG/` already proved the exact core stack on one CSV source (BGE-M3 dense+sparse, Qdrant named vectors, RRF fusion, bge-reranker-v2-m3, Groq `openai/gpt-oss-120b`). QABuddy is that stack productionized: 10 sources, per-source chunking, citations, server deploy, eval, backup. Team already knows the stack -> lower risk, direct code reuse.
...
Please make sure all data source should be uploaded able from UI in any format.
Please make sure all key of LLM should have settings to store the key and should get remove after refreshed the page ui.
...
- `/Users/jaivirsingh/Downloads/Advance_RAG/rag_core.py`: `embed()` (BGE-M3 dense+sparse), `Store` (Qdrant collection with named vectors, upsert/search), `rrf_fuse(k=60)`, `rerank()` (transformers workaround), `chunk_text()` -> becomes `app/core/`.
...
- Fixtures: `testcase/vwo_5000_test_cases.csv`, `/Users/jaivirsingh/Downloads/Product Requirements Document_(PRD)_VWO.com.pdf`.
